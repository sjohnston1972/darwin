...
const targetContract = {
  ...
  defaultBranch: 'main',
  application: {
    primaryUser: 'A knowledge worker coordinating project delivery.',
    ...
    navigation: ['Dashboard', 'My Work', 'Projects', 'Reports', 'Settings'],
    capabilities: [
      'dashboard work summary',
      'global task search',
      'direct assigned-work view',
    ...
    interfaceInventory: [
      {
        area: 'dashboard',
        purpose: 'Summarise assigned work and project health.',
        primaryActions: ['open assigned work', 'open a project'],
      },
      {
        area: 'my-work',
        purpose: 'Find and open assigned work.',
        primaryActions: ['search assigned tasks', 'open a task'],
      },
    ],
    routes: ['/dashboard', '/my-work', '/projects', '/reports', '/settings'],
...
export const createE2EBoundaryFetch =
  ...
  async (input, init) => {
    ...
    if (
      ...
      url.port === '5174' &&
      url.searchParams.has('darwin_deployment_verify')
    ) {
