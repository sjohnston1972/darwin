...
export class D1TelemetryRepository implements TelemetryRepository {
  ...
  async insertEvents(
    ...
    return {
      accepted,
      duplicates,
      quotaRejected: novelEvents.length - accepted,
    };
  }
    ...
    await this.database
      .prepare(
        `INSERT INTO repository_executions (
          ...
          execution_json, created_at, artifact_expires_at, record_expires_at,
          study_id
        ) VALUES (
          ?, ?, ?, ?, ?, ?, ?, ?, ?,
          (SELECT study_id FROM evidence_analyses WHERE analysis_id = ?)
        ...
        ON CONFLICT(execution_id) DO UPDATE SET
          ...
          study_id = COALESCE(
            excluded.study_id, repository_executions.study_id
          )`,
      )
      .bind(
        ...
        execution.createdAt,
        expiresAt(execution.createdAt, retentionPolicy().executionArtifactDays),
        expiresAt(execution.createdAt, retentionPolicy().fossilRecordDays),
        execution.analysisId,
      )
      .run();
  }
